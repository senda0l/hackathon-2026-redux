...
export class ZonesService {
  ...
  async getById(id: string) {
    return this.prisma.zone.findUnique({
      where: { id },
      include: { auctions: { include: { bids: true } }, tenders: true },
    });
  }
  ...
  async submitParcelRequest(
    zoneId: string,
    companyId: string,
    geometry: any,
    requestedType?: ZoneType,
  ) {
  ...
  async reviewRequest(
    ...
    await this.prisma.$transaction(async (tx) => {
      if (body.publicationType === PublicationType.AUCTION) {
        ...
        if (!body.tender)
          throw new BadRequestException('tender settings are required');
      ...
      await tx.zoneAuditLog.create({
        data: {
          ...
          reason:
            body.note ??
            `Approved request ${requestId} with ${body.publicationType}`,
        },
      });
    });
  ...
  async publishZoneDirect(
    ...
    if (zone.status !== ZoneStatus.AVAILABLE) {
      throw new BadRequestException(
        'Only AVAILABLE zones can be published directly',
      );
    }
