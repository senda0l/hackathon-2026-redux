<template>
  <div class="relative h-screen w-screen overflow-hidden">
    ...
    <!-- Top nav -->
    <div
      class="absolute top-4 left-4 right-4 flex items-center justify-between z-10"
    >
      <div
      ...
      <div class="flex gap-2">
        <router-link
          to="/auctions"
          class="bg-white rounded-xl shadow px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
        >
          Auctions
        </router-link>
        <router-link
        ...
        <template v-if="auth.isLoggedIn">
          <div
            class="hidden md:flex items-center bg-white rounded-xl shadow px-3 py-2 text-xs text-slate-600"
          >
            <span class="font-semibold text-slate-700 mr-1">
              {{ auth.companyName || 'Account' }}
            </span>
          ...
          <router-link
            v-if="auth.isGov"
            to="/admin"
            class="bg-blue-600 text-white rounded-xl px-4 py-2 text-sm font-medium"
          >
            Admin Panel
          </router-link>
          <button
            @click="auth.logout()"
            class="bg-slate-100 rounded-xl px-4 py-2 text-sm text-slate-600"
          >
            Logout
          </button>
        </template>
    ...
    <!-- Legend -->
    <div
      class="absolute bottom-8 left-4 bg-white rounded-xl shadow-lg p-4 z-10 text-sm space-y-2"
    >
    ...
    <transition name="slide">
      <div
        v-if="selectedZone"
        class="absolute top-20 right-4 w-72 bg-white rounded-2xl shadow-2xl p-5 z-10"
      >
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span
              :class="statusClass(selectedZone.status)"
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-slate-800"
              >${{ selectedZone.minPrice?.toLocaleString() }}</span
            >
          </div>
          <p
            v-if="selectedZone.description"
            class="text-slate-500 pt-1 border-t"
          >
        ...
        >
          <label class="block text-xs text-slate-500 mb-1"
            >Requested development type</label
          >
          <select
          ...
          >
            ...
            <option value="PUBLIC_INFRA">Public Infra</option>
          </select>
        </div>
        <p
          v-if="auth.isCompany && selectedZone.status === 'AVAILABLE'"
    ...
    <transition name="slide">
      <div
        v-if="auth.isCompany && (drawError || drawnParcel)"
        class="absolute bottom-8 right-4 w-80 bg-white rounded-2xl shadow-2xl p-5 z-10"
      >
        <div v-if="drawError" class="text-red-600 text-sm font-medium">
          {{ drawError }}
        </div>
        <div v-else-if="drawnParcel">
          ...
          <p class="text-slate-500 text-xs mb-4">
            This parcel is in an available zone. You can submit a bid request.
          </p>
          <label class="block text-xs text-slate-500 mb-1"
            >Requested development type</label
          >
          <select
            v-model="selectedRequestType"
            class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm mb-3"
          >
            <option value="RESIDENTIAL">Residential</option>
            <option value="INDUSTRIAL">Industrial</option>
            <option value="COMMERCIAL">Commercial</option>
          ...
          <p v-if="submitMsg" class="text-green-700 text-xs mt-2">
            {{ submitMsg }}
          </p>
          <p v-if="submitErr" class="text-red-600 text-xs mt-2">
            {{ submitErr }}
          </p>
        </div>
      </div>
    ...
    <!-- Company draw hint -->
    <div
      v-if="auth.isCompany && !drawnParcel && !drawError"
    ...
    <div
      ...
      class="absolute bottom-8 right-4 bg-white/90 rounded-xl px-4 py-2 z-10 text-sm text-slate-600 shadow"
    >
      Your current role is read-only here. Use a company account to submit
      parcel requests.
    </div>
  </div>
</template>
...
const selectedRequestType = ref<
  "RESIDENTIAL" | "INDUSTRIAL" | "COMMERCIAL" | "PUBLIC_INFRA"
>("RESIDENTIAL");
const apiBaseUrl =
...
const legend = [
  ...
  { label: "In Auction", color: "#ec4899" },
  { label: "In Tender", color: "#06b6d4" },
  { label: "Awarded", color: "#334155" },
...
function statusClass(status: string) {
  const map: Record<string, string> = {
    ...
    IN_AUCTION: "bg-pink-100 text-pink-700",
    IN_TENDER: "bg-cyan-100 text-cyan-700",
    AWARDED: "bg-slate-700 text-white",
...
async function submitParcel() {
  ...
  try {
    await api.post(`/zones/${selectedDrawZone.value.id}/requests`, {
      ...
      requestedType: selectedRequestType.value,
    });
